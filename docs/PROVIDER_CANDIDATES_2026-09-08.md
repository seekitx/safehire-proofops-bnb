...
> 16:00 更新：ChainHelix 网格订单 #56741 已返回有效结果；主网状态、签名、原始字节指纹及计算均独立核验通过，当前 SUBMITTED，未完成结算。详见 [实际交付记录](CHAINHELIX_DELIVERY_56741_2026-09-08.zh-CN.md)。下面保留此前筛选过程，其中“尚无当前交付”是历史结论；LP 替代仍未完成。
...
## 15:35–15:46 北京时间：更换供应商复核

用户要求先不发送原供应商问题单，转而寻找替代。本轮未发送问题单、未签名、未付款、未把候选标为已完成接入。

### ChainHelix Grid Trader：签名通过，仍不是当前交付通过

- 对具体任务 `{"budgetUsd":1000,"levels":5,"price":750.0,"spanPct":2}` 取得实时完整报价。750 是假设价格，1000 是计算用假设预算，不是当前币价或转账金额。
- 使用 SafeHire 现有验证器独立验证签名；再读取 ERC-8004 注册表，确认 #269224 的当前代理钱包与签名钱包一致。没有放宽验证器。
- 当前服务费 0.5 U，另有链上手续费。预计完成 600 秒，报价有效期 900 秒；这些都不是交付保证。
- 官方 `/api/agents/gridtrader/hire/plan` 返回 `jobLifetimeSeconds=777600`，即默认 9 天。不能把预计 10 分钟完成理解为 10 分钟可退款。
- 官方网格执行技能预览返回 unknown skill；缺少必填 price 的报价探针仍 accepted。故没有获得当前任务的免费有效交付，也不能以 accepted 证明输入已正确执行。
- 官方购买页交易准备可以生成，但其默认验收条件与本地严格审核报价不同；不得把两者混用。未签署返回的交易。
- 它可以作为网格计算候选，不能替代原 LP 仓位管理任务。SafeHire 现有 0.1 U 和任务包装约束尚未为该候选完成独立适配，未更改全局费用。

### Genesis RangeKeeper：免费指定任务实测未通过

[本次公开结果](https://genesis-marketplace-one.vercel.app/jobs/job_mtsd0mhu_nr7jyw) 来自免费 Get plan，无钱包操作。输入指定第三方公开 PancakeSwap V3 BNB/USDT 仓位 #7319347，要求准确绑定仓位，并考虑约 0.88 美元仓位价值与假设 2 美元调仓成本。

结果虽实际读到了 WBNB/USDT 的链上数据，主结论却写成 CAKE/USDT，使用默认组合金额，未解释成本高于仓位价值，且将 fee=500 显示为 5%。判定为本次任务质量不合格，不推荐付费。

本轮原始报价、独立验证、注册表和交易准备保留在本地忽略目录 `.data/provider-replacement-2026-09-08/`。结论仍是：有可继续评估的另一运营方，尚无已验收、可无缝替换原付费 LP 任务的新供应商。原订单 #56733 仍应单独跟踪，资金不能挪到新供应商。
