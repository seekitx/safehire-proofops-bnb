...
Browser precision review: uint128 liquidity is parsed from the original JSON token and kept as an exact numeric token with text editing, then serialized as the exact original integer; original decimal strings in source observations remain strings. The server task format and hash domain remain unchanged. Browsers lacking lossless JSON primitives refuse the operation with an update-browser message. Desktop and mobile-width Chromium tests save, reload and export `327142007496340585` without changing any digit.

Evidence-export review: signed/hashed server documents are downloaded using the exact response bytes. Re-serializing JSON in JavaScript can change `2.0` to `2` and invalidate a Python-canonical bundle digest even when the normalized task hash still matches. Browser regression now runs the offline bundle verifier on the downloaded file, including the complete bundle digest, rather than trusting the server-provided integrity flag alone.

Unsigned preparation normalizes submitted financial inputs with the same strict category model as the frozen task before comparing them. This allows JSON's equivalent whole-decimal spellings (`2` and `2.0`) while rejecting changed values, extra fields, invalid types and boolean-as-number input. The signed task remains the server-normalized frozen task with the original hash.
